...
class CakeShopListUi extends StatelessWidget {
  const CakeShopListUi({super.key});

  // ตัวแปรเก็บรูปที่จะเอาไปแสดงที่ CarouselSlider ---------------------
  static final List<String> imgShopLogo = [
  ...
  static final List<CakeShop> cakeShops = [
    CakeShop(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFEEE9EF),
      appBar: AppBar(
      ...
      body: Column(
        children: [
          ...
          CarouselSlider(
            ...
            items: imgShopLogo.map((imgPath) {
              return ClipRRect(
                borderRadius: BorderRadius.circular(12),
                child: _SafeAssetImage(path: imgPath, width: double.infinity),
              );
            }).toList(),
          ),
                  ...
                  trailing: IconButton(
                    onPressed: () {
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => CakeShopDetailUi(shop: shop),
                        ),
                      );
                    },
